...
use interval_desktop_core::replay_quality::{self, BadgeTone, ChannelBadge};
...
/// The `.panel` section frame with its uppercase mono header (`Panel.tsx`).
pub(crate) fn panel(
    title: &'static str,
    quality: Option<&ChannelBadge>,
    ...
    div()
        ...
        .child(
            div()
                ...
                .children(quality.map(quality_badge)),
        )
        .child(div().flex_1().min_h_0().overflow_hidden().child(body))
}

/// `QualityBadge.tsx`: a bordered uppercase micro-badge in the panel header.
fn quality_badge(badge: &ChannelBadge) -> Div {
    let (border, text) = match badge.tone {
        BadgeTone::Ready => (
            theme::blend(theme::MINT(), theme::PANEL_HI(), 0.6),
            theme::MINT(),
        ),
        BadgeTone::Degraded => (
            theme::blend(theme::AMBER(), theme::PANEL_HI(), 0.7),
            theme::AMBER(),
        ),
        BadgeTone::Missing => (theme::LINE(), ui::faint()),
    };
    div()
        .border_1()
        .border_color(border)
...
struct PanelsData {
    ...
    race_control_quality: ChannelBadge,
    events: Vec<ReplayEvent>,
    events_state: EventFeedState,
    weather_quality: ChannelBadge,
...
fn collect(app: &IntervalApp) -> Option<PanelsData> {
    ...
    Some(PanelsData {
        ...
        race_control_quality: replay_quality::quality_badge(&snapshot.race_control.quality),
        events,
        events_state,
        weather_quality: replay_quality::quality_badge(&snapshot.weather.quality),
...
pub fn side_panels(
    ...
    let Some(data) = data else {
        return column
            .child(panel("RACE CONTROL", None, div()).flex_1().min_h(rems(6.0)))
            .child(panel("EVENT FEED", None, div()).flex_1().min_h(rems(6.0)))
            .child(panel("WEATHER", None, div()).flex_none().h(rems(7.0)))
            .child(panel("DERIVED METRICS", None, div()).flex_1().min_h(rems(6.0)));
    };
    column
        .child(
            panel(
                "RACE CONTROL",
                Some(&data.race_control_quality),
                race_control_body(&data.race_control),
            )
            .flex_1()
            .min_h(rems(6.0)),
        )
        .child(
            panel(
                "EVENT FEED",
                None,
                event_feed_body(&data.events, data.events_state),
            )
            .flex_1()
            .min_h(rems(6.0)),
        )
        .child(
            panel(
                "WEATHER",
                Some(&data.weather_quality),
                weather_body(data.weather.as_deref()),
            )
            .flex_none()
            .h(rems(7.0)),
        )
        .child(
            panel("DERIVED METRICS", None, derived_body(&data.derived))
                .flex_1()
                .min_h(rems(6.0)),
        )
}
    ...
    div()
        ...
        .children(metrics.chunks(2).map(|pair| {
            div().flex().flex_row().gap_4().children(pair.iter().map(|metric| {
                div()
                    .flex_1()
                    .child(div().text_color(ui::faint()).child(metric.label))
                    .child(
                        div()
                            .text_sm()
