...
Releases are GitHub draft releases with one archive per platform, built by `.github/workflows/release.yml`. To cut one, merge `dev` into `latest` and push:
...
The push triggers the `release` workflow, which:
