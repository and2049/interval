...
function Assert-BahrainReplayReady($backendUrl) {
    $sessions = Invoke-RestMethod -Uri "$backendUrl/api/sessions?meeting_key=1229"
    $bahrain = $sessions | Where-Object { $_.session.session_key -eq 9472 } | Select-Object -First 1
    if (-not $bahrain) {
        throw "Bahrain session 9472 was not seeded. Start the backend once to run migrations and fixture seeding."
    }
    if (-not $bahrain.replay_ready) {
        throw "Bahrain replay is not cached yet. Run the app and choose INGEST + OPEN for session 9472 before running this cached MVP smoke."
    }
}

function Assert-ReplayStream($url, $seconds) {
    $client = [System.Net.Http.HttpClient]::new()
    ...
    try {
        ...
        while ((Get-Date) -lt $deadline -and -not ($seenMetadata -and $seenSnapshot)) {
            ...
            if ($currentEvent -eq "metadata") {
                ...
                Assert-Equal $payload.session.session_key 9472 "stream metadata session"
                $seenMetadata = $true
            } elseif ($currentEvent -eq "snapshot") {
                Assert-Equal $payload.contract_version "replay.v1" "stream snapshot contract"
                Assert-Equal $payload.cursor.session_key 9472 "stream snapshot session"
                Assert-Truthy ($payload.timing.rows.Count -ge 20) "stream timing rows"
                Assert-Equal $payload.timing.quality "ready" "stream timing quality"
                Assert-Equal $payload.track.quality "projected" "stream track quality"
                $seenSnapshot = $true
            }
        }
...
try {
    ...
    $env:INTERVAL_REBUILD_SESSION_ON_START = "9472"
    ...
    Assert-BahrainReplayReady $backendUrl

    $metadata = Invoke-RestMethod -Uri "$backendUrl/api/sessions/9472/replay/metadata"
    Assert-Equal $metadata.contract_version "replay.v1" "metadata contract"
    Assert-Equal $metadata.session.session_key 9472 "metadata session"
    Assert-Equal $metadata.meeting.meeting_key 1229 "metadata meeting"
    Assert-Equal $metadata.meeting.name "Bahrain Grand Prix" "metadata meeting name"
    ...
    $snapshot = Invoke-RestMethod -Uri "$backendUrl/api/sessions/9472/replay/snapshot?t=600"
    Assert-Equal $snapshot.contract_version "replay.v1" "snapshot contract"
    Assert-Equal $snapshot.cursor.session_key 9472 "snapshot session"
    Assert-Truthy ($snapshot.timing.rows.Count -ge 20) "timing rows"
    Assert-Equal $snapshot.timing.quality "ready" "timing quality"
    Assert-Equal $snapshot.track.map_mode "projected" "Bahrain map mode"
    Assert-Equal $snapshot.track.quality "projected" "track quality"
    ...
    Assert-CountAtLeast $snapshot.derived_metrics 20 "derived metrics"
    Assert-Equal $snapshot.derived_metrics[0].label "3-lap avg" "derived metric label"
    Assert-CountAtLeast $snapshot.track.positions 20 "projected track positions"
    $projectedPositions = $snapshot.track.positions | Where-Object { $_.source -eq "projected" -and $_.quality -eq "projected" }
    Assert-CountAtLeast $projectedPositions 20 "projected-quality driver positions"
    Assert-HttpJsonError "$backendUrl/api/sessions/9472/replay/snapshot?t=NaN" 400 "finite number" "invalid snapshot cursor"

    $geometry = Invoke-RestMethod -Uri "$backendUrl/api/sessions/9472/track/geometry"
    Assert-Equal $geometry.contract_version "replay.v1" "geometry contract"
    Assert-Equal $geometry.source "curated_static" "Bahrain geometry source"
    Assert-Equal $geometry.quality "ready" "Bahrain geometry quality"
    Assert-Equal $geometry.map_mode "projected" "Bahrain geometry map mode"
    Assert-CountAtLeast $geometry.centerline 20 "Bahrain geometry centerline"
    Assert-CountAtLeast $geometry.inner_edge 20 "Bahrain geometry inner edge"
    Assert-CountAtLeast $geometry.outer_edge 20 "Bahrain geometry outer edge"
    Assert-Truthy ($geometry.circuit_length -gt 1000) "Bahrain geometry circuit length"

    $events = Invoke-RestMethod -Uri "$backendUrl/api/sessions/9472/replay/events"
    ...
    $proxiedMetadata = Invoke-RestMethod -Uri "$frontendUrl/api/sessions/9472/replay/metadata"
    Assert-Equal $proxiedMetadata.session.session_key 9472 "proxied metadata session"
    Assert-Equal $proxiedMetadata.meeting.name "Bahrain Grand Prix" "proxied metadata meeting"

    $proxiedEvents = Invoke-RestMethod -Uri "$frontendUrl/api/sessions/9472/replay/events"
    Assert-ReplayEvents $proxiedEvents "proxied events"

    Write-Output "MVP smoke passed: Bahrain replay metadata, meeting context, snapshot, events, geometry, stream, and Vite proxy are healthy."
} finally {
